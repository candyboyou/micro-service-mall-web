<template>
  <div class="page-detail">
    <h2 class="page-title">分类 - <span class="fw-semi-bold">编辑</span></h2>
    <!--    上面的大表-->
    <b-row>
      <b-col>
        <Widget>
...
import {
  ...
  getCategoryListByLevel, getParentCategoryListByLevel,
...
let attributeIdToNameMap = new Map();
...
export default {
  ...
  data() {
    return {
      ...
      tempIconUrl: null,
      newKeyWord: null,
      isShowPlaceholderOfAttributes: true,
      isShowPlaceholderOfKeyWords: true,
      name:null
    };
  },
  created() {
  ...
  methods: {
    fillCategoryDetailParam() {
      let categoryId = this.$route.query.id;
      if (categoryId === null || categoryId === undefined) {
        return;
      }
      getCategoryDetailById({id: categoryId}).then(response => {
        let data = response.data;
        let saveParam = this.saveParam;
        saveParam.name = data.name;
        saveParam.sort = data.sort;
        saveParam.isShow = data.showStatusId;
        saveParam.place = data.placeId;
        saveParam.iconUrl = data.iconUrl;
        saveParam.keyWords = data.keyWords;

        if (saveParam.keyWords !== undefined && saveParam.keyWords.length > 0) {
          this.isShowPlaceholderOfKeyWords = false;
        }
        saveParam.description = data.description;
        saveParam.relationAttributes = data.relationAttributes;

        if (saveParam.relationAttributes !== undefined && saveParam.relationAttributes.length !== 0) {
          this.isShowPlaceholderOfAttributes = false;
        }
        this.oldRelationAttributes = data.relationAttributes;
        this.notRelationAttributes = data.notRelationAttributes;
        let levelId = data.levelId;
        if (levelId != null) {
          this.getSameCategoryListByLevel(levelId);
          saveParam.level = levelId;
          saveParam.parentId = data.parentId;
          if (saveParam.parentId != null) {
            this.getSubCategoryListById(saveParam.parentId);
          }
        }
    ...
    fillNewAttribute(newValue) {
      ...
      for (let key of newValue) {
        let obj = {}
        obj.id = key;
        obj.label = attributeIdToNameMap.get(key.toString());
